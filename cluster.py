...
parser.add_argument("--val_path", type=str, default=None, help="path to data")
...
def main():
    ...
    # build datasets
    train_folder = args.data_path
    val_folder = args.val_path
    ...
    _, val_targets, val_embeddings, val_paths = getEmb(val_loader, model, len(val_dataset.imgs))
    train_targets = train_targets.tolist()

    # clustering
    print("Clustering features ...")
    deepcluster = Kmeans(args.num_classes, nredo=30,dump_path=dump_path)
    ...
    val_labels_acc = deepcluster.labels[len(train_dataset.imgs):]
    ...
    print("train_csd nmi {:.4f}".format(nmi_train))
    print("train_csd acc {:.4f}".format(acc_train))
    # clustering metric
    nmi_val = normalized_mutual_info_score(val_targets, val_labels_acc)
    acc_val, _ = reAssignSingle(np.array(val_targets), np.array(val_labels_acc), num_classes=args.num_classes, )
    print("train nmi {:.4f}".format(nmi_val))
    print("train acc {:.4f}".format(acc_val))
...
def save(train_paths, val_paths, dump_path, result):
    ...
    with open(os.path.join(dump_path, "traincsd_labeled.txt"), "w") as f:
        f.write("\n".join(train_labeled))
